...
app.get('/get-cookie', (req, res) => {
  ...
  // req yra iš naršyklės atsiųstas užklausos objektas
  // req.cookies - iš naršyklės atsiųsto užklausos objekto, cookies objektas
  // req.cookie.manoSaldainis - iš naršyklės atsiųsto užklausos objekto, cookies objekto, manoSaldainis reikšmė


  const cookie = req.cookies.manoSaldainis || 'Nėra cookie';
  // console.log(`Gautas cookie iš kliento: ${cookie}`);
...
app.get('/set-cookie', (req, res) => {
  ...
  // options - papildomos nustatymų parinktys, pvz., maxAge (gyvavimo laikas ms), httOnly(ar cookie pasiekiamas tik per HTTP)


  res.cookie('manoSaldainis', 'CukrinisBebras', { maxAge: (1000 * 60 * 60 * 24) * 10, httpOnly: true /* httpOnly: true */ }); // cookie gyvavimo laikas 10 dienų
...
app.post('/login', (req, res) => {
  ...
  if (username === testUsername && password === testPassword) {
    ...
    res.cookie('loginStatus', 'loggedIn', { maxAge: 100 * 60 * 60 * 24, httpOnly: true });
